...
--
-- Hôte : 127.0.0.1:3306
-- Généré le : mar. 01 avr. 2025 à 15:06
...
CREATE TRIGGER `before_delete_commande` BEFORE DELETE ON `commande` FOR EACH ROW BEGIN
    DECLARE etape_livraison VARCHAR(50);
    -- Récupérer l'étape associée à la commande
    SELECT s.etape INTO etape_livraison 
    FROM suivi s 
    WHERE s.id = OLD.id
    LIMIT 1;
    -- Vérifier si la commande est "livrée"
    IF etape_livraison = 'livrée' THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Impossible de supprimer une commande déjà livrée';
    END IF;
END
$$
DELIMITER ;
...
-- Déclencheurs `suivi`
--
DROP TRIGGER IF EXISTS `after_update_suivi_livree`;
DELIMITER $$
CREATE TRIGGER `after_update_suivi_livree` AFTER UPDATE ON `suivi` FOR EACH ROW BEGIN
    IF NEW.etape = 'livrée' THEN
        INSERT INTO exemplaire (dateAchat, id)
        SELECT c.dateCommande, cd.id
...
INSERT INTO `utilisateur` (`id`, `nom`, `prenom`, `login`, `motDePasse`, `idService`) VALUES
('1', 'Du Pont', 'Jean', 'jdupont', '446b5a14e90b3d6139c1db2afebf4eb12af48dcf67b0179f435515ab03587841', '1'),
('2', 'Martin', 'Sophie', 'smartin', 'a88fbb3c8ba5bc960b5eaf0c390cd1e9395bc342d64aadd1354b7994e0ea0721', '2'),
('3', 'Lemoine', 'Alice', 'alemoine', '0ecfacc5d325cd0063e40d626b092466896d5e36a7f8f56560af169ac18dbfbd', '3');
